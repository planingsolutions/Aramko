    ...
    <div class="card-body">
        <div class="row">
            <div class="col-sm-5">
                ...
                @lang('labels.backend.access.Home.Recent Projects') <small class="text-muted">{{ __('labels.backend.access.Home.Mainsectionf.active') }}</small>
                </h4>
            </div>
            <!--col-->
        </div>
...
<script>
  ...
  $('#Main_section').DataTable({
        processing: false,
        serverSide: true,
        ajax: "{{ route('admin.furnitureTable') }}",
        columns: [
        { data: 'id', name: 'id' },
...
function deleteFunc(id){
    ...
    $.ajax({
    type:"POST",
    url: "{{ url('admin/Delete-furniture') }}",
    data: { id: id },
    dataType: 'json',
    success: function(res){
...
// submit Button Function
    ...
    $('#CompanyFormCreate').submit(function(e) {
            ...
            $.ajax({
            type:'POST',
            url: "{{ url('admin/Create-furniture')}}",
            data: formData,
            cache:false,
            contentType: false,
            processData: false,
