...
@include('backend.Page3.includes.breadcrumb-linksServ')
@endsection
        ...
        <div class="row">
            <div class="col-sm-5">
                <h4 class="card-title mb-0">
                    {{ __('labels.backend.access.Home.testimonials') }} <small class="text-muted">{{ __('labels.backend.access.Home.Mainsectionf.active') }}</small>
                </h4>
            </div>
            <!--col-->
        </div>
                    ...
                    <table id="Main_section" class="table">
                        <thead>
                            ...
                            <tr>
                                ...
                                <th>{{ trans('labels.backend.access.Home.aboutsec1.name') }}</th>
                                <th>{{ trans('labels.backend.access.Home.aboutsec1.TestCareer') }}</th>
                                <th>{{ trans('labels.backend.access.Home.aboutsec1.Test opinion') }}</th>
...
<!-- name -->
<div class="form-group">
                    <label for="Tittle" class="col-sm-12 control-label">{{ trans('labels.backend.access.Home.aboutsec1.name') }}</label>
                    <div class="col-sm-12">
                        <input type="text" class="form-control" id="name" name="name" placeholder="{{trans('forms.Main.Placeholder.Tittle')}}" maxlength="255" required>
                    </div>
                   
                </div>
<!-- Job Tittle -->
                <div class="form-group">
                    <label for="Tittle" class="col-sm-12 control-label">{{ trans('labels.backend.access.Home.aboutsec1.TestCareer') }}</label>
                    <div class="col-sm-12">
                        <input type="text" class="form-control" id="jobTittle" name="jobTittle" placeholder="{{trans('forms.Main.Placeholder.Tittle')}}" maxlength="255" required>
                    </div>
                   
                </div>
<!-- Customer opinion -->
                <div class="form-group">
                    <label for="Tittle" class="col-sm-12 control-label">{{ trans('labels.backend.access.Home.aboutsec1.Test opinion') }}</label>
                    <div class="col-sm-12">
                        <input type="text" class="form-control" id="opinion" name="opinion" placeholder="{{trans('forms.Main.Placeholder.Tittle')}}" maxlength="255" required>
                    </div>
...
// Edit-function
    function editFunc(id){
        ...
        $.ajax({
            type:"POST",
            url: "{{url('admin/edittestimonials')}}",
            data:{id:id},
            dataType:'json',
            success:function(res)
            {
                ...
                $('#name').val(res.Tittle.lang);
                $('#jobTittle').val(res.Paragraph1.lang);
                $('#opinion').val(res.Paragraph2.lang);
...
function deleteFunc(id){
    ...
    $.ajax({
    type:"POST",
    url: "{{ url('admin/destroytestimonials') }}",
    data: { id: id },
    dataType: 'json',
    success: function(res){
...
// submit Button Function
    $('#CompanyForm').submit(function(e) {
            ...
            $.ajax({
            type:'POST',
            url: "{{ url('admin/storetestimonials')}}",
            data: formData,
            cache:false,
            contentType: false,
            processData: false,
