    ...
    <div class="card-body">
        <div class="row">
            <div class="col-sm-5">
                ...
                @lang('labels.backend.access.Home.medical facilities') <small class="text-muted">{{ __('labels.backend.access.Home.Mainsectionf.active') }}</small>
                </h4>
            </div>
            <!--col-->
        </div>
...
<script>
  ...
  $('#Main_section').DataTable({
        processing: false,
        serverSide: true,
        ajax: "{{ route('admin.medicalfacilities') }}",
        columns: [
...
// Edit-function
    function editFunc(id){
        ...
        $.ajax({
            type:"POST",
            url: "{{url('admin/Edit-medicalfacilities')}}",
            data:{id:id},
            dataType:'json',
...
function deleteFunc(id){
    ...
    $.ajax({
    type:"POST",
    url: "{{ url('admin/Delete-medicalfacilities') }}",
    data: { id: id },
    dataType: 'json',
    success: function(res){
...
// submit Button Function
    $('#CompanyForm').submit(function(e) {
            ...
            $.ajax({
            type:'POST',
            url: "{{ url('admin/Store-medicalfacilities')}}",
            data: formData,
            cache:false,
            contentType: false,
            processData: false,
...
$('#CompanyFormCreate').submit(function(e) {
        ...
        $.ajax({
        type:'POST',
        url: "{{url('admin/Create-medicalfacilities')}}",
        data:formData,
        cache:false,
        contentType: false,
        processData: false,
