        ...
        <div class="row">
            <div class="col-sm-5">
                <h4 class="card-title mb-0">
                    {{ __('labels.backend.access.Home.Servicess') }} <small class="text-muted">{{ __('labels.backend.access.Home.Mainsectionf.active') }}</small>
                </h4>
            </div>
            <!--col-->
        </div>
                    ...
                    <table id="Main_section" class="table">
                        <thead>
                            <tr>
                                ...
                                <th>{{ trans('labels.backend.access.Home.aboutsec1.P1') }}</th>
                                <th>{{ trans('labels.backend.access.Home.aboutsec1.P2') }}</th>
...
<!-- Images -->
                <div class="form-group">
                    ...
                    <div class="col-sm-12">
                        <input type="file" class="form-control" accept=".png" id="Images" name="Images[]" placeholder="{{trans('forms.Main.Placeholder.Images')}}" maxlength="255" style="height: calc(2.5em + 0.75rem + -4px);">
                    </div>
                </div> 
<!-- Tittle -->
                <div class="form-group">
                    <label for="Tittle" class="col-sm-12 control-label">{{ trans('labels.backend.access.Home.aboutsec1.P1') }}</label>
                    <div class="col-sm-12">
...
<!-- Description -->
<div class="form-group">
                    <label for="Description" class="col-sm-12 control-label">{{ trans('labels.backend.access.Home.aboutsec1.P2') }}</label>
                    <div class="col-sm-12">
                        <input type="text" class="form-control" id="Paragraph2" name="Paragraph2" placeholder="{{trans('forms.Main.Placeholder.Description')}}" maxlength="255" required>
                    </div>
                </div>
...
<script>
  ...
  $('#Main_section').DataTable({
        processing: false,
        serverSide: true,
        ajax: "{{ route('admin.ServicessTable') }}",
        columns: [
    ...
    function editFunc(id){
        ...
        $.ajax({
            ...
            success:function(res)
            {
                ...
                $('#Paragraph2').val(res.Paragraph1.lang);
...
function deleteFunc(id){
    ...
    $.ajax({
    type:"POST",
    url: "{{ url('admin/Delete-servicess') }}",
    data: { id: id },
    dataType: 'json',
    success: function(res){
...
// submit Button Function
    ...
    $('#CompanyFormCreate').submit(function(e) {
            ...
            $.ajax({
            type:'POST',
            url: "{{ url('admin/Create-servicess')}}",
            data: formData,
            cache:false,
            contentType: false,
            processData: false,
